...
/*
  This is experimenting with bypassing the store (as the rest of the app uses) so that
  this Content Card can use the same translation approach whether or not it is embedded.
  The goal was to prevent pulling the entire store for each card. Pulling translationSet 
...
export default {
  ...
  methods: {
    t(slug) {
      const id = translationSlugMap[slug]
      if (!id) console.warn(`slug not found ${slug}`)
